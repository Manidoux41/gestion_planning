...
import { Check, Clock3, FileText, LogIn, Trash2, Undo2 } from "lucide-react";
...
export default async function TimesheetPage() {
  ...
  return <AppShell activePath="/timesheet" user={user}><div className="content-wrap app-page"><div className="page-heading"><div><p className="eyebrow">Suivi du temps</p><h1>{t("timesheet.title")}</h1><p className="page-subtitle">{t("timesheet.subtitle")}</p></div><div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>{isAdmin && <Link href="/payroll" className="outline-button"><FileText size={16} /> Gérer la paie &amp; avances</Link>}{isAdmin && <ManualTimeEntryForm nannies={nannies.map((nanny) => ({ id: nanny.id, name: `${nanny.firstName} ${nanny.lastName}` }))} />}<ExportTimesheetButton entries={rows} /></div></div>
    ...
    {nannies[0] && <section className="table-card"><div className="card-heading"><div><p className="eyebrow">{isAdmin ? `${nannies[0].firstName} ${nannies[0].lastName}` : "Mon quota"}</p><h2>Quota d&apos;heures du mois</h2></div><div style={{ display: "flex", alignItems: "center", gap: 12 }}>{isAdmin && <Link href="/payroll" className="outline-button" style={{ padding: "6px 12px", fontSize: 11 }}><FileText size={14} /> Voir le calcul de paie</Link>}<strong>{Math.min(100, Math.round((monthHours / plannedHours) * 100 || 0))}%</strong></div></div><div className="quota-bar"><div className="quota-bar-fill" style={{ width: `${Math.min(100, (monthHours / plannedHours) * 100 || 0)}%` }} /></div><p className="page-subtitle">{formatHours(monthHours)} validées sur {formatHours(plannedHours)} prévues · estimation utile pour anticiper la prochaine paie.</p></section>}
