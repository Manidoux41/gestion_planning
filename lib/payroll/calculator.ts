export type PayrollInput = { weeklyHours: number; monthlySalary: number; workedHours: number; overtimeRate: number; bonuses?: number; deductions?: number };
...
export function calculatePayroll(input: PayrollInput): PayrollSummary {
  const plannedHours = getMonthlyHours(input.weeklyHours);
