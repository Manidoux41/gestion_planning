...
export function ExportTimesheetButton({ entries }: { entries: TimeEntry[] }) {
  ...
  function exportCsv() {
    const rows = [["Jour", "Date", "Période", "Arrivée", "Départ", "Durée", "Statut"], ...entries.map((entry) => [entry.day, entry.date, entry.period, entry.arrival, entry.departure, entry.duration, entry.status])];
