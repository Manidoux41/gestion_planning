...
import { KeyRound, Mail, Plus, Settings2, ShieldCheck, ShieldOff, X } from "lucide-react";
import type { NannyRecord } from "@/lib/db/domain-types";
import { createNannyAccountAction, updateNannyRateAction, type NannyActionState } from "@/app/actions/nanny";
  ...
  return <>
    ...
    <div className="entity-grid">
      {nannies.map((nanny) => <article className="entity-card" key={nanny.id}>
        <div className="entity-avatar sage">{nanny.name.split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase()}</div>
        <div className="entity-card-body">
          ...
          <button type="button" className="outline-button" style={{ marginTop: 12, width: "100%", justifyContent: "center" }} onClick={() => setEditing(nanny)}><Settings2 size={15} /> Modifier la rémunération</button>
        </div>
      </article>)}
